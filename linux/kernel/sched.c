...
//		next = 0;
		...
		if ( c )
			break;
		/* MOREFIX: !!!! 这里的next必须加上？？ 全部都在休眠怎么办？？？ 加上c==-1
		 		if ( c == -1 || c && next )
			break;  MOREFIX: !!!! 这里的next必须加上？？ 全部都在休眠怎么办？？？ 加上c==-1
		 */
...
#ifdef __DEBUG_
	printk("Switch: %d -> %d\n", current->pid, next?next->pid:current->pid);
...
/* Clear NT, so that we won't have troubles with that later on */
	...
	task[0]->th.eip = init_eip;/* MOREFIX: */
	ltr(0);
	lldt(0);
